...
parser.add_argument("--data", default='/home/egk204/PycharmProjects/clone-type-iv/data/')
parser.add_argument('--root' , default='/home/egk204/PycharmProjects/clone-type-iv/src/models/gnn/')
...
astdict,vocablen,vocabdict=createast(args)
...
def test(dataset):
    ...
    res = str(now)+':     '+args.bench+' Precision: '+str(p)+'  Recall: '+str(r)+'  F1: '+str(f1) +'\n'
    
    f = open('results_ggnn.txt', 'a')
    f.write(res)
    f.close()
    return results
...
for epoch in epochs:# without batching
    ...
    devfile=open(args.root+'results/'+args.graphmode+'_ggnn_dev_epoch_'+str(epoch+1),mode='w')
    for res in devresults:
        devfile.write(str(res)+'\n')
    devfile.close()
    testresults=test(testdata)
    resfile=open(args.root+'results/'+args.graphmode+'_epoch_'+str(epoch+1),mode='w')
    for res in testresults:
        resfile.write(str(res)+'\n')
    resfile.close()
    torch.save(model.state_dict(), args.root+'/saved_models/ggnnn_'+args.bench+'_'+str(epoch))
