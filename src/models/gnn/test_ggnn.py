...
parser.add_argument("--saved_model_path", default=project_root+'ggnn')
...
model=gmn.models.GGNN(vocablen=77535,embedding_dim=100,num_layers=num_layers,device=device).to(device)
...
pd.concat([pairs_df, df_res], axis=1).to_csv(args.root+str(+args.bench)+'_ggnn_predictions.csv', index=None)