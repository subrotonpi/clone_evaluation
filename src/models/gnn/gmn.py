...
parser.add_argument("--storage", default='gptcb')
...
parser.add_argument("--bench",default='gptcb')
...
def test(dataset):
    ...
    # f = open(f'results_gmn_{args.bench}.txt', 'a')
    # f.write(res)
    # f.close()
    return res
...
for epoch in epochs:# without batching
    ...
    # devfile=open(args.root+'results/'+args.graphmode+'_dev_epoch_'+str(epoch+1),mode='w')
    # for res in devresults:
    #     devfile.write("dev: "+str(res)+'\n')
    # devfile.close()
    ...
    #     resfile.write("test: "+str(res)+'\n')
    # resfile.close()
    
    torch.save(model.state_dict(), args.root+'saved_models/gmn_'+args.graphmode+'_'+args.bench+'_'+str(epoch))