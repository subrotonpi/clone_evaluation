...
def createast(args):
    ...
    alltokens=[]
    import pandas as pd
    p = args.current_dataset[2] #args.data+args.bench+'/data.jsonl'
    df = jsonl_to_df(p)
    df=df.sample(frac=1)
...
def creategmndata(args, id,treedict,vocablen,vocabdict,device):
    print("modified for testing only... uncomment to get train, valid data")
    dataset_path = args.data +'/'+ args.bench +'/'
    k = ['id1', 'id2', 'label']
    # trainlist = pd.read_csv(dataset_path+'train.csv', names=k)
    # trainlist=trainlist.replace(0, -1)
    # validlist = pd.read_csv(dataset_path+'valid.csv', names=k)
    # validlist=validlist.replace(0, -1)
    #testlist = pd.read_csv('dataset_path+'test.csv'', names=k)
    
    testlist = pd.read_csv('/home/egk204/PycharmProjects/clone-type-iv/data/scb/scb_pairs.csv', names=k)
    testlist=testlist.replace(0, -1)
    testlist=testlist.sample(frac=1)
    ...
    return testdata #traindata, validdata, testdata 
