...
def read_examples(args, filename):
    ...
    logger.info("***** Running Read examples from filename *****")
    print('processing'+filename)
    import pandas as pd
    #ds = '~/PycharmProjects/code-clone-multilingual/storage/storage/alternative/'
    ...
    codes = jsonl_to_df(args.code)
    codes = codes= dict(codes[['idx', 'func']].values)
    filename = '/home/egk204/PycharmProjects/clone-type-iv/data/scb/scb_pairs.csv'
    pairs = pd.read_csv(filename ,names=['id1', 'id2', 'label'])#.sample(frac=.7)
    examples=[]
    for index, row in pairs.iterrows():
        #logger.info(str(row.id1)+" . "+str(row.id2))
        code1 = codes[int(row.id1)]
        code2 = codes[int(row.id2)]
        lang1 = get_catagory_id('java')
        lang2 = get_catagory_id('java')
        task = str(row.label)
        examples.append(
            Example(
                source1=code1,
...
def main():
    ...
    if args.do_test:
        ...
        mtype = args.code.split('/')[-2]
        logger.info(str(mop)+' '+str(mtype))
        ...
        writeresult = mtype+' '+mop+' ' +str(round(temp_best_precision, 3)) + str(round(temp_best_recall, 3))+str(round(temp_best_f1, 3))+'\n'
        #'recall: ' + str(round(temp_best_recall, 3)) + ' precision:' + str(round(temp_best_precision, 3)) + ' F1:'+str(round(temp_best_f1, 3))+' tau:' + str(tau) + ' threshold:' +             str(round(best_threshold, 1))+'\n' 
    
        f = open('result_mutation.txt','a+')
        f.write(writeresult)
        f.close()
...
def prepare_dataset(args, filename, tokenizer, portion=1):
    train_examples = read_examples(args, filename)
