...
def read_examples(filename):
    ...
    codes = pd.read_pickle('/home/egk204/PycharmProjects/code-clone-multilingual/storage/dataset_small/codes.pkl')#alternative/codes_alt.pkl')
    #pairs = pairs = pd.read_csv('~/PycharmProjects/code-clone-multilingual/experimental/C4/dataset/c4_all_pos.csv' ,names=['id1', 'id2', 'label'])        
    storage = '~/PycharmProjects/code-clone-multilingual/experimental/C4/'
    pairs = pd.read_csv(filename ,names=['id1', 'id2', 'label']).sample(frac=.7)
    examples=[]
    for index, row in pairs.iterrows():
        logger.info(str(row.id1)+" . "+str(row.id2))
        f1 = codes[codes.id == row.id1]
        f2 = codes[codes.id == row.id2]

        code1 =  f1.code.item() 
        code2 = f2.code.item() 

        lang1 = get_catagory_id(f1.language_extension.item())
        lang2 = get_catagory_id(f2.language_extension.item())
        #if f1.task.values[0] == f2.task.values[0]:
        task = str(f1.task.item())
        
        #print(type(task), type(lang1), type(code1))   
